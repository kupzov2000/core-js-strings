...
function removeFirstOccurrences(str, value) {
  ...
  return str.includes(value) ? str.slice(0, start).concat(str.slice(end)) : str;
}
...
function countVowels(str) {
  return str
    .split('')
    .filter((elem) => /[aeiouyAEIOUY]/.test(elem))
    .join('').length;
}
...
function extractNameFromTemplate(value) {
  return value
    .split(' ')
    .map((word) =>
      word.includes('!') ? word.slice(0, [word.indexOf('!')]) : word
    )
    .slice(1, 3)
    .join(' ');
}
...
function encodeToRot13(str) {
  ...
  const firstHalfAlphabet = alphabet
    .slice(0, 13)
    .concat(alphabet.slice(26, 39));
  const secondAlphabet = alphabet.slice(13, 26).concat(alphabet.slice(39, 52));

  const notLetters = /[^a-zA-Z ]/.test(str);
  ...
  const rot13 = str
    ...
    .map((word) =>
      word
        .split('')
        .map((char) =>
          firstHalfAlphabet.includes(char)
            ? secondAlphabet[firstHalfAlphabet.indexOf(char)]
            : firstHalfAlphabet[secondAlphabet.indexOf(char)]
        )
        .join('')
    )
    .join(' ');

  return notLetters ? rot13.concat(str[str.length - 1]) : rot13;
}
...
function getCardId(value) {
  const cardSuits = '♣♦♥♠';
  const cardNumber = 'A234567891JQK';

  return (
    cardNumber.indexOf(value.slice(0, 1)) +
    cardSuits.indexOf(value.slice(-1)) * 13
  );
}
