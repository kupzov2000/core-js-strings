...
function removeFirstOccurrences(str, value) {
  const startStr = str.indexOf(value);
  const endStr = startStr + value.length;
  const result =
    startStr === -1
      ? str
      : str.slice(0, startStr) + str.slice(endStr, str.length);
  return result;
}
...
function removeLastOccurrences(str, value) {
  const startStr = str.lastIndexOf(value);
  const endStr = startStr + value.length;
  const result =
    startStr === -1
      ? str
...
function findLongestWord(sentence) {
  ...
  for (let i = 0; i < arrStr.length; i += 1) {
    if (currStr.length < arrStr[i].length) currStr = arrStr[i];
  }
  return currStr;
}
